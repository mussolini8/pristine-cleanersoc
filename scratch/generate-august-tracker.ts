...
function mapCategory(service: string, freq: string): string {
...
const residentialEntries: string[] = [];
...
for (let i = 1; i < lines.length; i++) {
  ...
  computeBookingFormulas({
    ...
    serviceCategory: ${JSON.stringify(category as any)},
...
const commercialEntries = importedCommercialAccounts.map((acc) => {
  ...
  if (acc.name === "The Harper Wedding Venue") {
    ...
    monthlyHours = (Number(acc.hours) || 3) * 4;
    visits = 4;
