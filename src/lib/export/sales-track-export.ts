...
export async function exportComprehensiveTrackerToXLSX(
  ...
  if (commercialBookings.length > 0) {
    const commRows: ExportRow[] = commercialBookings.map((b) => ({
      ...
      "Full Name": b.clientName,
      "Sub-Total": b.subTotal,
      "Processing Fee": b.processingFee !== undefined ? b.processingFee : (b.merchantFee || 0),
      "Final Amount": b.finalAmount,
      "Tip": b.tip,
      "Frequency": b.frequency,
      "Teams": b.cleanerTeam,
      "Team Earnings": b.teamEarningsWithoutTips,
      "Labour %": `${(b.laborPct * 100).toFixed(2)}%`,
      "Hourly Labor": b.hourlyLabor || "",
      "Supplies": b.supplies !== undefined ? b.supplies : 0,
      "PC Earnings": b.pcEarnings,
      "PC Profit %": `${(b.pcProfitPct * 100).toFixed(2)}%`,
  ...
  const appendFilteredSheet = (sheetName: string, filterFn: (b: ServiceBookingRow) => boolean) => {
    ...
    if (subset.length > 0) {
      const rows: ExportRow[] = subset.map((b) => ({
        ...
        "Frequency": b.frequency,
        "Teams": b.cleanerTeam,
        "Team": b.teamEarningsWithoutTips,
        "Labour %": `${(b.laborPct * 100).toFixed(2)}%`,
        "PC Earnings": b.pcEarnings,
        "PC Profit %": `${(b.pcProfitPct * 100).toFixed(2)}%`,
        "Duration": b.durationHours,
